...
(function (global) {
  ...
  const IDENTIFIER_PATTERNS = {
    stableId:
      /^(?:user|account|customer|member|profile|subject)[_.-]?id$|^uid$/i,
    uuid: /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i,
    emailKey: /^(?:e[_.-]?mail|emailAddress)$/i,
    email: /^[^\s@]{1,64}@[^\s@.]{1,63}(?:\.[^\s@.]{1,63})+$/,
    usernameKey: /^(?:user[_.-]?name|login[_.-]?name|screen[_.-]?name)$/i,
    username: /^[\p{L}0-9][\p{L}0-9._@-]{2,79}$/u,
    phoneKey: /^(?:phone|phoneNumber|mobile|mobileNumber|telephone)$/i,
    phone: /^\+?[0-9][0-9 ()-]{6,20}$/,
    personNameKey:
      /^(?:full[_.-]?name|display[_.-]?name|first[_.-]?name|last[_.-]?name|given[_.-]?name|family[_.-]?name)$/i,
    personName: /^[\p{L}][\p{L}' -]{1,79}$/u,
  };
  ...
  function looksLikeOpaqueToken(value) {
    ...
    if (
      ...
      return false;
    // Absolute and relative application routes are not secrets. For example,
    // window.unlockPassword = "users/unlock-user" has a sensitive-looking
    // name but its value is plainly a route. Keep conventional base64 values
    // containing '/' eligible when they also contain '+' or '='.
    if (
      v.startsWith('/') ||
      (v.includes('/') &&
        !v.includes('+') &&
  ...
  function identifierType(key, value) {
    const k = (key || '').trim();
    const v = (value || '').trim();
    if (IDENTIFIER_PATTERNS.uuid.test(v))
      return IDENTIFIER_PATTERNS.stableId.test(k)
        ? 'user/account identifier'
        : 'stable UUID identifier';
    if (
      IDENTIFIER_PATTERNS.emailKey.test(k) &&
      IDENTIFIER_PATTERNS.email.test(v)
    )
      return 'email address';
    if (
      IDENTIFIER_PATTERNS.phoneKey.test(k) &&
      IDENTIFIER_PATTERNS.phone.test(v)
    )
      return 'telephone number';
    if (
      IDENTIFIER_PATTERNS.personNameKey.test(k) &&
      IDENTIFIER_PATTERNS.personName.test(v)
    )
      return 'person name';
    if (
      IDENTIFIER_PATTERNS.usernameKey.test(k) &&
      IDENTIFIER_PATTERNS.username.test(v)
    )
      return 'username';
    return null;
  }

  function identifierFinding(keyName, value, apiName, path, type) {
    return mk(
      'Identifying data stored in ' + apiName + ': ' + path,
      apiName === 'sessionStorage' ? 'LOW' : 'MEDIUM',
      'CERTAIN',
      'storage',
      apiName + '.' + path + ' = ' + truncate(value),
      'The browser confirmed that this value contains a ' +
        type +
        '. It is not treated as an authentication secret, but scripts running on the origin ' +
        ...
        (apiName === 'localStorage'
          ...
          : 'sessionStorage remains readable for the lifetime of this browsing context.'),
      {
        key: 'identifier:' + apiName + ':' + type + ':' + value,
        location: apiName + '.' + path,
        baseTitle: 'Identifying data stored in ' + apiName,
      }
    );
  }
  ...
  function walkJsonForTokens(
    ...
    if (typeof value === 'string') {
      ...
      const type = identifierType(keyContext, value);
      if (type) {
        findings.push(
          identifierFinding(keyContext, value, apiName, path, type)
        );
        return;
      }
  ...
  function analyzeStorage(storageDump, apiName) {
    ...
    for (const [key, value] of Object.entries(storageDump || {})) {
      ...
      const type = identifierType(key, value);
      if (type) {
        findings.push(identifierFinding(key, value, apiName, key, type));
        continue;
      }
