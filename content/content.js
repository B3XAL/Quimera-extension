 ...
 * on every user-authorized page is the whole point of this file, it must not depend on a
 * cross-context resource fetch succeeding just to get started. Burp scope is only a UI filter;
 * it never suppresses collection or bridge delivery. Keep these
...
(function () {
  ...
  if (globalThis.__quimeraContentLoaded) return;
  globalThis.__quimeraContentLoaded = true;

  const MSG = {
    COLLECTED: 'quimeraCollected',
    REQUEST_SNAPSHOT: 'quimeraRequestSnapshot',
    RETRY_DELIVERY: 'quimeraRetryDelivery',
  };
  ...
  async function collectAndSend(reason) {
    ...
    // Registration and browser host permissions already decide where this content script may
    // run. Do not apply a second stored-scope filter here: it can become stale after permission
    // changes and silently discard a page the user has explicitly authorized.
    ...
    if (fingerprint === lastFingerprint || fingerprint === inFlightFingerprint)
      return;
    inFlightFingerprint = fingerprint;
    try {
      const delivery = await api.runtime.sendMessage({
        type: MSG.COLLECTED,
        payload,
        findings,
      });
      // A snapshot is only deduplicated after the Java bridge actually accepted it. If the page
      // loaded before pairing, Burp was temporarily unreachable, or the token was being changed,
      // retain it as pending so visibility/storage/periodic events retry the exact same evidence.
      if (delivery?.bridgeStatus === 'ok') lastFingerprint = fingerprint;
    } catch (e) {
      ...
      if (inFlightFingerprint === fingerprint) inFlightFingerprint = '';
    }
  }
  ...
  api.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.type === MSG.RETRY_DELIVERY) {
      schedule('bridge-settings-changed');
      return false;
    }
