...
(async function () {
  ...
  /** Keeps collectors registered only for exact hosts explicitly authorized by the user. */
  async function syncQuimeraContentScripts(options) {
    ...
    const hosts = [
      ...new Set((options.scopeHosts || []).map(normalizeScopeEntry)),
    ].filter(Boolean);
    if (!hasGlobalAccess && !hosts.length) return;
    const matches = hasGlobalAccess
      ? ['<all_urls>']
      : [...new Set(hosts.flatMap(scopeEntryOrigins))];
    const mainWorld = browserDetector.isFirefox()
      ? {
          id: ids[0],
          matches,
          js: ['content/mainworld-loader.js'],
      ...
      : {
          id: ids[0],
          matches,
          js: ['content/inject-mainworld.js'],
          runAt: 'document_start',
          world: 'MAIN',
          allFrames: true,
        };
    await api.scripting.registerContentScripts([
      mainWorld,
      {
        id: ids[1],
        matches,
        js: ['content/engine.js', 'content/content.js'],
        runAt: 'document_idle',
        allFrames: false,
      },
    ]);
  ...
  function handleMessage(request, sender, sendResponse) {
    ...
    switch (request.type) {
      ...
      // ── Quimera additions ──────────────────────────────────────────────
      case MSG.COLLECTED: {
        // Sent automatically by content.js on every page (passive capture).
        const tabId = sender.tab && sender.tab.id;
        if (tabId === undefined) return false;
        ingestQuimeraSnapshot(tabId, request.payload, request.findings).catch(
          error => console.error('[Quimera] ingest error', error)
        );
        return false; // fire-and-forget, no response expected by content.js
      }
      ...
      case MSG.SET_OPTIONS: {
        (async () => {
          await storeQuimeraOptions(request.options);
          await syncQuimeraContentScripts(request.options);
          if (request.options.burpScopeSyncEnabled) {
            browserDetector
              .getApi()
              .alarms.create('quimera-burp-scope-sync', { periodInMinutes: 1 });
          } else {
            await browserDetector
              .getApi()
              .alarms.clear('quimera-burp-scope-sync');
          }
