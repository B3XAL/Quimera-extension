...
(async function () {
  ...
  // ------ Bridge widget: enable/port/token/authenticated status in the popup header. ------
  initBridgeWidget();
  ...
  async function initBridgeWidget() {
    ...
    const tokenInput = document.getElementById('quimera-bridge-widget-token');
    ...
    const setPopoverOpen = open => {
      ...
      document.body.classList.toggle('quimera-bridge-popover-open', open);
    };
    ...
    saveBtn.addEventListener('click', async () => {
      ...
      if (enabledInput.checked && token.length < 32) {
        statusEl.textContent =
          'Not paired: paste the token shown by Quimera Burp.';
        statusEl.style.color = '#cf000f';
        return;
      }
      ...
      try {
        if (enabledInput.checked) {
          ...
          const capturePermission = {
            origins: ['<all_urls>', 'http://127.0.0.1/*', 'http://localhost/*'],
          };
          // This must be the first awaited operation in the Save click handler.
          // Firefox otherwise considers the user activation consumed and rejects
          // permissions.request(), even if the earlier await was only contains().
          // Requesting an already-granted optional permission simply returns true.
          const granted = await api.permissions.request(capturePermission);
          if (!granted) {
            statusEl.textContent =
              'Website and bridge permissions were not granted.';
            statusEl.style.color = '#cf000f';
            return;
          }
        }
        // Merge into the full options object; scope and collectors survive untouched.
        ...
        const result = await sendMessage(MSG.SET_OPTIONS, { options: updated });
        if (!result?.success) {
          throw new Error(result?.error || 'The settings were not saved.');
        }
        saved = true;
      } catch (error) {
        statusEl.textContent =
          'Could not save bridge settings: ' + (error?.message || error);
        statusEl.style.color = '#cf000f';
      } finally {
        saveBtn.disabled = false;
        if (saved) await refreshWidget();
      }
    });
    ...
    async function refreshWidget() {
      ...
      const reachable = !!(ping && ping.connected);
      if (!reachable) {
        dot.textContent = '●';
        dot.style.color = '#cf000f';
        statusEl.style.color = '#cf000f';
        statusEl.textContent =
      ...
      const paired = auth?.status === 'ok';
      dot.textContent = '●';
      dot.style.color = paired ? '#1e824c' : '#cf000f';
      statusEl.style.color = paired ? '#1e824c' : '#cf000f';
      statusEl.textContent = paired
        ? 'Connected and paired with Quimera.'
        : 'Reachable, but not paired: token missing or incorrect.';
    }
