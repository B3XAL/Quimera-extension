...
document.addEventListener('DOMContentLoaded', async event => {
  ...
  async function initQuimeraBridgeOptions() {
    ...
    enabledInput.addEventListener('change', async () => {
      if (await save()) checkBridgeStatus();
    });
    ...
    async function save() {
      ...
      // Enabling the bridge means continuous browser-side capture, so request global website
      // access in this same trusted Save/change gesture. Previously only loopback was requested;
      // an empty Burp scope therefore left Chrome with origins: [] and no content script, while
      // adding a scope host happened to request that one origin and made it look scope-dependent.
      if (enabledInput.checked) {
        origins.push('<all_urls>', 'http://127.0.0.1/*');
      }
    ...
    /** Pings the bridge, then, only if reachable, checks the token against an authenticated
     * endpoint. PING_BRIDGE alone only proves the port is open, deliberately unauthenticated, so
     * it must never be shown as "Connected" on its own, that would say "paired" when the token is
     * actually missing or wrong. */
    async function checkBridgeStatus() {
      if (!enabledInput.checked) {
        setBridgeStatus('Bridge disabled.', 'neutral');
        return;
      }
      if (tokenInput.value.trim().length < 32) {
        setBridgeStatus(
          'Not paired: paste the token shown by Quimera Burp.',
          'error'
        );
        return;
      }
      setBridgeStatus('Checking…', 'neutral');
      const ping = await sendMessage(MSG.PING_BRIDGE);
      if (!ping || !ping.connected) {
        setBridgeStatus(
      ...
      const auth = await sendMessage(MSG.CHECK_BRIDGE_AUTH);
      if (auth && auth.status === 'ok') {
        setBridgeStatus('Connected and paired with Quimera', 'success');
        return;
      }
      setBridgeStatus(
        'Reachable, but not paired (' +
          (auth?.error || 'check the token') +
          ')',
        'error'
      );
    ...
    /** Asks for the pairing token, saves and re-checks if one is provided; does nothing on
     * Cancel. Only called when the bridge answered but rejected the current token. */
    async function promptForToken() {
      const pasted = window.prompt(
        "Quimera is reachable but the pairing token is missing or wrong. Paste the token shown by the Quimera Burp extension's Browser Bridge dialog (or Cancel to leave it as is):"
      );
      if (pasted === null) return;
      if (pasted.trim().length < 32) {
        setBridgeStatus(
          'That token looks too short, try again from "Check now".',
          'error'
        );
        return;
      }
    ...
    function setBridgeStatus(text, state) {
      ...
      statusEl.style.color =
        state === 'success' ? '#1e824c' : state === 'error' ? '#cf000f' : '';
    }
